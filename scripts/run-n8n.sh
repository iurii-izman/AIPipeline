...
podman run -d \
  ...
  ${NOTION_INBOX_DATABASE_ID:+-e NOTION_INBOX_DATABASE_ID="$NOTION_INBOX_DATABASE_ID"} \
  ${NOTION_SPECS_DATABASE_ID:+-e NOTION_SPECS_DATABASE_ID="$NOTION_SPECS_DATABASE_ID"} \
  ${NOTION_SPEC_TEMPLATE_ID:+-e NOTION_SPEC_TEMPLATE_ID="$NOTION_SPEC_TEMPLATE_ID"} \
  ...
  ${PROJECTS_CONFIG:+-e PROJECTS_CONFIG="$PROJECTS_CONFIG"} \
  ${DEFAULT_PROJECT_KEY:+-e DEFAULT_PROJECT_KEY="$DEFAULT_PROJECT_KEY"} \
  ${WF5_RBAC_ALLOWED_CHAT_IDS:+-e WF5_RBAC_ALLOWED_CHAT_IDS="$WF5_RBAC_ALLOWED_CHAT_IDS"} \
  ${WF5_RBAC_ALLOWED_USER_IDS:+-e WF5_RBAC_ALLOWED_USER_IDS="$WF5_RBAC_ALLOWED_USER_IDS"} \
  ${WF5_RBAC_ALLOWED_USERNAMES:+-e WF5_RBAC_ALLOWED_USERNAMES="$WF5_RBAC_ALLOWED_USERNAMES"} \
  ${WF5_PRIVILEGED_COMMANDS:+-e WF5_PRIVILEGED_COMMANDS="$WF5_PRIVILEGED_COMMANDS"} \
  ${INTAKE_INGEST_URL:+-e INTAKE_INGEST_URL="$INTAKE_INGEST_URL"} \
  ${INTAKE_INGEST_TOKEN:+-e INTAKE_INGEST_TOKEN="$INTAKE_INGEST_TOKEN"} \
  ${INTAKE_PUBLIC_BASE_URL:+-e INTAKE_PUBLIC_BASE_URL="$INTAKE_PUBLIC_BASE_URL"} \
  ${INTAKE_AUTO_CONVERT:+-e INTAKE_AUTO_CONVERT="$INTAKE_AUTO_CONVERT"} \
  ${INTAKE_AUTO_CONVERT_CONFIDENCE:+-e INTAKE_AUTO_CONVERT_CONFIDENCE="$INTAKE_AUTO_CONVERT_CONFIDENCE"} \
  "$IMAGE"
