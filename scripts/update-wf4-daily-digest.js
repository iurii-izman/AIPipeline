...
const workflow = {
  ...
  nodes: [
    ...
    },
    {
      ...
      position: [440, 0],
      parameters: {
        jsCode: `const items = $input.all().map((i) => i.json || {});\nconst toKey = (value) => String(value || '').trim().toLowerCase();\n\nlet projects = [];\ntry {\n  const parsed = JSON.parse(String($env.PROJECTS_CONFIG || '{\"projects\":[]}'));\n  projects = Array.isArray(parsed) ? parsed : (Array.isArray(parsed.projects) ? parsed.projects : []);\n} catch {\n  projects = [];\n}\n\nconst byProjectId = new Map();\nconst byProjectKey = new Map();\nfor (const p of projects) {\n  const key = toKey(p?.key);\n  const projectId = toKey(p?.linearProjectId);\n  if (key) byProjectKey.set(key, p);\n  if (projectId) byProjectId.set(projectId, p);\n}\n\nconst stateBuckets = { started: 0, completed: 0, canceled: 0, backlog: 0, unstarted: 0, other: 0 };\nconst perProject = new Map();\n\nfor (const issue of items) {\n  const stateType = toKey(issue.state?.type);\n  const bucket = stateType && stateBuckets[stateType] !== undefined ? stateType : 'other';\n  stateBuckets[bucket] += 1;\n\n  const pid = toKey(issue.project?.id);\n  const pkey = toKey(issue.project?.key || issue.project?.name);\n  const known = byProjectId.get(pid) || byProjectKey.get(pkey) || null;\n  const displayKey = toKey(known?.key) || pkey || 'unmapped';\n  const displayLabel = String(known?.label || issue.project?.name || issue.project?.key || 'Unmapped');\n\n  if (!perProject.has(displayKey)) {\n    perProject.set(displayKey, {\n      key: displayKey,\n      label: displayLabel,\n      started: 0,\n      completed: 0,\n      backlog: 0,\n      other: 0,\n      total: 0,\n    });\n  }\n  const row = perProject.get(displayKey);\n  row.total += 1;\n  if (bucket === 'started') row.started += 1;\n  else if (bucket === 'completed') row.completed += 1;\n  else if (bucket === 'backlog' || bucket === 'unstarted') row.backlog += 1;\n  else row.other += 1;\n}\n\nconst today = new Date().toISOString().slice(0, 10);\nconst allTotal = items.length;\nconst allDone = stateBuckets.completed;\nconst allStarted = stateBuckets.started;\nconst allOpen = Math.max(0, allTotal - allDone);\n\nconst lines = [];\nlines.push('*All projects*');\nlines.push('• Done: ' + allDone + '/' + allTotal);\nlines.push('• Started: ' + allStarted);\nlines.push('• Open: ' + allOpen);\n\nconst sorted = Array.from(perProject.values()).sort((a, b) => a.label.localeCompare(b.label));\nif (sorted.length) {\n  lines.push('');\n  lines.push('*By project*');\n  for (const p of sorted) {\n    const open = Math.max(0, p.total - p.completed);\n    lines.push('• ' + p.label + ' (' + p.key + '): done ' + p.completed + '/' + p.total + ', started ' + p.started + ', open ' + open);\n  }\n}\n\nconst text = '📋 *Daily digest* (' + today + ')\\n' + lines.join('\\n');\nreturn [{ json: { text, date: today, lines: lines.join('\\n') || 'No issues', summary: { allTotal, allDone, allStarted, allOpen } } }];`,
      },
    },
    {
      id: "telegram-wf4",
