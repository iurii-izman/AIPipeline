...
return [{ json: { ...$json, signatureValid: valid, signatureReason: valid ? 'ok' : 'signature-mismatch' } }];`,
    ...
    },
    {
      ...
      name: "If GitHub signature valid",
...
return [{ json: { linearFailed: true, rateLimited, reason, failureClass } }];`,
    ...
    },
    {
      ...
      parameters: {
        ...
        sendBody: true,
        specifyBody: "json",
        jsonBody: "={{ { chat_id: $env.TELEGRAM_CHAT_ID || 'YOUR_CHAT_ID', message_thread_id: Number($json.threadId || 0) || undefined, text: $json.text || 'WF-2 notification' } }}",
        options: {},
      },
    },
    {
...
return [{ json: { telegramFailed: true, rateLimited, reason: msg } }];`,
  ...
  connections: {
    "GitHub PR Webhook": { main: [[{ node: "Verify GitHub signature", type: "main", index: 0 }]] },
    "Verify GitHub signature": { main: [[{ node: "If GitHub signature valid", type: "main", index: 0 }]] },
    "If GitHub signature valid": { main: [[{ node: "Extract PR payload", type: "main", index: 0 }], []] },
    ...
    "DLQ: park Linear failure": { main: [[{ node: "Prepare Telegram payload", type: "main", index: 0 }]] },

    "Set merged + updated": { main: [[{ node: "Prepare Telegram payload", type: "main", index: 0 }]] },
    "Set merged fallback": { main: [[{ node: "Prepare Telegram payload", type: "main", index: 0 }]] },
    "Set PR opened": { main: [[{ node: "Prepare Telegram payload", type: "main", index: 0 }]] },
    "Prepare Telegram payload": { main: [[{ node: "Telegram: send", type: "main", index: 0 }]] },
