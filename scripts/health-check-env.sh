...
for var in GITHUB_PERSONAL_ACCESS_TOKEN LINEAR_API_KEY LINEAR_TEAM_ID NOTION_TOKEN NOTION_SPRINT_LOG_DATABASE_ID TELEGRAM_BOT_TOKEN TELEGRAM_CHAT_ID N8N_BASIC_AUTH_USER N8N_BASIC_AUTH_PASSWORD N8N_API_KEY SENTRY_DSN SENTRY_AUTH_TOKEN SENTRY_ORG_SLUG SENTRY_PROJECT_SLUG OPENAI_API_KEY CLOUDFLARED_TUNNEL_TOKEN CLOUDFLARE_PUBLIC_BASE_URL GITHUB_OWNER GITHUB_REPO GITHUB_WORKFLOW_STAGING GITHUB_WORKFLOW_PRODUCTION OTEL_ENABLED OTEL_EXPORTER_MODE OTEL_EXPORTER_OTLP_ENDPOINT WF5_RBAC_ALLOWED_CHAT_IDS WF5_RBAC_ALLOWED_USER_IDS WF5_RBAC_ALLOWED_USERNAMES WF5_PRIVILEGED_COMMANDS; do
  if [[ -n "${!var:-}" ]]; then
    echo "  $var: set"
  else
    echo "  $var: MISSING"
