...
2. Ограниченная зрелость AI quality gates (offline, dataset 54, без online drift gates).  
...
- Eval: offline dataset `evals/datasets/sentry-severity-alpha.json` (54 labeled cases), gate thresholds enforced by `scripts/run-ai-eval.js` and ADR rollout criteria.
...
| 8 | Idempotency dispatch volatile | `GitHubClient` uses in-memory `Set` | Medium | Medium | P2 | Добавить TTL cache file/kv для dedupe | Перейти на persistent idempotency store | Duplicate dispatch rate < 1% при restart scenario |
| 9 | DLQ хранится в static data n8n | WF-7 uses `$getWorkflowStaticData('global')` | High | Medium | P1 | Периодический export + backup verification | Перевести DLQ storage в durable DB/object store | DLQ survives restart/upgrade, replay audit complete |
...
| 16 | AI eval ограничен offline dataset 54 | `evals/datasets/sentry-severity-alpha.json`, `run-ai-eval.js` | High | High | P0 | Расширить dataset до >150 кейсов, incl. hard negatives | Создать eval harness v2 (offline+online scorecards) | Регрессии ловятся до merge/release, FN critical <= target |
