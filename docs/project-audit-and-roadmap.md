...
| `src/modules/github-client/index.ts` | Typed GitHub API client + workflow dispatch | High | Persistent idempotency реализована для dispatch; не покрыты все mutation paths |
...
| 9 | DLQ replay path migration завершён частично | WF-7 replay убран из static data и переведен на app durable API; остается расширить durable pattern на все replay сценарии | Medium | Low | P1 | Поддерживать replay через app API + evidence | Перевести все recovery сценарии на единый durable orchestration слой | DLQ survives restart/upgrade, replay audit complete |
...
| 20 | Telegram command center RBAC требует операционного закрепления | WF-5 allowlist checks для privileged команд внедрены; нужна формализация ролей и регулярный audit trail review | Medium | Low | P2 | Поддерживать allowlist chat/user checks + denied evidence | Политика ролей/операций + audit trail per command | Privileged команды исполняются только authorized actors |
